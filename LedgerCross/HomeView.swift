...
struct HomeView: View {
    ...
    var body: some View {
            ...
            VStack(spacing: 18) {
                ...
                NavigationLink(destination: HowToPlayView()) {
                    infoTile(title: "How to Play", subtitle: "Rules + digit combo cheat sheet",
                             icon: AnyView(GridMarkIcon(color: LCTheme.teal)))
                }
                .buttonStyle(PlainButtonStyle())
        ...
        .navigationBarItems(trailing:
            NavigationLink(destination: SettingsView()) {
                GearIcon(color: LCTheme.ink).frame(width: 24, height: 24)
            }
        )
    }
    ...
    private var continueCard: some View {
        if let lp = store.lastPlayed, let diff = LedgerDifficulty(rawValue: lp.difficulty) {
            let puzzle = LedgerCatalog.shared.puzzle(difficulty: diff, index: lp.index)
            ...
            NavigationLink(destination: GameView(puzzle: puzzle, title: "\(diff.title) · #\(lp.index + 1)")
                ...
                HStack(spacing: 14) {
                    ZStack {
                        RoundedRectangle(cornerRadius: 14).fill(LCTheme.teal)
    ...
    private var dailyCard: some View {
        let today = LedgerCatalog.dayKey(Date())
        ...
        return NavigationLink(destination: DailyView()) {
            HStack(spacing: 14) {
                ZStack {
                    RoundedRectangle(cornerRadius: 14).fill(LCTheme.amber.opacity(0.16))
                    FlameIcon(color: LCTheme.amber).frame(width: 30, height: 30)
                }
                ...
                VStack(alignment: .leading, spacing: 4) {
                    HStack(spacing: 6) {
                        Text("Daily Challenge")
                            ...
                            .foregroundColor(LCTheme.ink)
                        if done {
                            CheckIcon(color: LCTheme.teal).frame(width: 16, height: 16)
                        }
                    }
                ...
                Spacer()
                ChevronIcon(color: LCTheme.inkSoft).frame(width: 20, height: 20)
            }
            .padding(14)
            .background(LCTheme.card)
            .clipShape(RoundedRectangle(cornerRadius: 18))
    ...
    private var packsGrid: some View {
        ...
        return LazyVGrid(columns: cols, spacing: 12) {
            ForEach(LedgerDifficulty.allCases, id: \.rawValue) { diff in
    ...
    private func packTile(_ diff: LedgerDifficulty) -> some View {
    ...
    private func difficultyDots(_ diff: LedgerDifficulty) -> some View {
        HStack(spacing: 3) {
            ForEach(0..<4, id: \.self) { i in
                Circle()
                    .fill(i <= diff.rawValue ? LCTheme.amber : LCTheme.line)
    ...
    private var divider: some View {
        Rectangle().fill(LCTheme.line).frame(width: 1, height: 34)
    }
        ...
        HStack {
            ...
            if showChevron {
                Text("All")
                    ...
                    .foregroundColor(LCTheme.teal)
                ChevronIcon(color: LCTheme.teal).frame(width: 16, height: 16)
            }
        }
        .padding(.top, 4)
    }
    ...
    private func infoTile(title: String, subtitle: String, icon: AnyView) -> some View {
        HStack(spacing: 14) {
            ZStack {
                RoundedRectangle(cornerRadius: 12).fill(LCTheme.tealSoft)
                icon.frame(width: 26, height: 26)
            }
            .frame(width: 48, height: 48)
            VStack(alignment: .leading, spacing: 3) {
                Text(title).font(.system(size: 16, weight: .bold)).foregroundColor(LCTheme.ink)
                Text(subtitle).font(.system(size: 12, weight: .medium)).foregroundColor(LCTheme.inkSoft)
            }
            Spacer()
            ChevronIcon(color: LCTheme.inkSoft).frame(width: 20, height: 20)
        }
    ...
    var body: some View {
        GeometryReader { geo in
            ZStack(alignment: .leading) {
                Capsule().fill(LCTheme.line.opacity(0.5))
                Capsule().fill(LCTheme.teal)
...
struct PackRow: View {
    ...
    var body: some View {
        ...
        return HStack(spacing: 14) {
            ZStack {
                RoundedRectangle(cornerRadius: 14).fill(LCTheme.tealSoft)
            ...
            VStack(alignment: .leading, spacing: 5) {
                Text(difficulty.title).font(.system(size: 18, weight: .bold)).foregroundColor(LCTheme.ink)
                Text(difficulty.subtitle).font(.system(size: 13, weight: .medium)).foregroundColor(LCTheme.inkSoft)
                ProgressBar(value: total > 0 ? Double(done) / Double(total) : 0).frame(height: 7).frame(maxWidth: 180)
            }
            Spacer()
            VStack(alignment: .trailing, spacing: 4) {
                Text("\(done)/\(total)").font(.system(size: 15, weight: .bold, design: .rounded)).foregroundColor(LCTheme.teal)
                ChevronIcon(color: LCTheme.inkSoft).frame(width: 18, height: 18)
            }
        }
        .padding(14)
        .background(LCTheme.card)
