            ...
            VStack(spacing: 0) {
                ...
                HStack(spacing: 8) {
                    ForEach(0..<steps, id: \.self) { i in
                        Capsule()
                            .fill(i == step ? LCTheme.teal : LCTheme.line)
...
private struct MiniCell: View {
    ...
    var body: some View {
        ZStack {
            ...
            case .entry(let v):
                Rectangle().fill(highlight ?? LCTheme.paper)
                Text("\(v)").font(.system(size: side * 0.5, weight: .semibold, design: .rounded))
                    .foregroundColor(highlight == LCTheme.redSoft ? LCTheme.red : LCTheme.ink)
            case .empty:
                Rectangle().fill(highlight ?? LCTheme.paper)
            case .note(let marks):
                Rectangle().fill(LCTheme.paper)
                VStack(spacing: 0) {
        ...
        .overlay(Rectangle().stroke(LCTheme.ink.opacity(0.8), lineWidth: 0.8))
    }
}
...
private struct ClueCellsIllustration: View {
    var body: some View {
        ...
        .overlay(Rectangle().stroke(LCTheme.ink, lineWidth: 2))
    }
}
...
private struct FillRuleIllustration: View {
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 0) {
                MiniCell(kind: .clue(across: 0, down: 0))
                MiniCell(kind: .entry(4), highlight: LCTheme.tealSoft)
                MiniCell(kind: .entry(5), highlight: LCTheme.tealSoft)
            }
        }
        .background(LCTheme.ink)
        .overlay(Rectangle().stroke(LCTheme.ink, lineWidth: 2))
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 0) {
                ...
                MiniCell(kind: .entry(3), highlight: LCTheme.redSoft)
                MiniCell(kind: .entry(3), highlight: LCTheme.redSoft)
            }
        }
        .background(LCTheme.ink)
